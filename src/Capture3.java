...
public class Capture3 extends JFrame implements ComponentListener, MouseListener{
	...
	public Capture3()
	{
        ...
        // 範囲を指定してキャプチャ
        Rectangle bounds = new Rectangle(100, 100, 850, 300);	//ウィンドウの初期状態
	...
	public void save() {
		try {
			AbstractSave as = new ExcelSave(leaderShelf);
			as.save();
	 ...
	 * public void thread_start(){
		...
		new Thread(() -> {
			while(threadRunning) {
				...
				copyScreen();
				copyScreen_Leader();
				tm.SetMat_Leader(image_Leader);
				tm.SetMat(image);
				System.out.println("エルフ" + tm.elf());
				System.out.println("ろいやる" + tm.royal());
				System.out.println("ウィッチ" + tm.witch());
				System.out.println("ドラゴン" + tm.dragon());
				System.out.println("ネクロ" + tm.nekura());
				System.out.println("ヴァンプ" + tm.vampire());
				System.out.println("ビショップ" + tm.bishop());
				System.out.println("ネメシス" + tm.nemesis());
				if	(tm.Win_Maching()) {
					System.out.println("Win");
					leaderShelf.Win();
					player.Win(leaderShelf);
	...
	public void thread_start_Leader(){
		...
		new Thread(() -> {
			while(threadRunning_Leader) {
				...
				if(tm_l.Leader_Maching() != -1 && Matching_Leader == false) {
					leaderShelf.chngeindex(tm_l.Leader_Maching());
					Matching_Leader = true;
				}
				System.out.println(tm_l.Leader_Maching());
				if (time <= interval) {
					try {
						Thread.sleep(interval - time);
					} catch (InterruptedException e) {
